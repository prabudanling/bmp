    ...
    <footer className="mt-auto bg-slate-900 text-slate-300">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
            ...
            <div className="mt-4 flex items-center gap-2">
              {socials.map((s) => (
                <a
                  ...
                  title={s.label}
                  aria-label={s.label}
                  className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-slate-400 transition-all duration-300 hover:-translate-y-1 hover:bg-teal-500/20 hover:text-teal-300"
                >
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-xs">
            {(categories?.items || []).slice(0, 6).map((c) => (
              <li key={c.id}>
                <button
                  className="inline-block transition-all duration-200 hover:translate-x-1 hover:text-teal-400"
        ...
        <div>
          ...
          {settings?.whatsapp && (
            <button
              className="mt-4 inline-flex items-center gap-2 rounded-lg bg-green-600 px-3 py-2 text-xs font-semibold text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-green-500 hover:shadow-lg hover:shadow-green-600/25"
              onClick={() =>
                window.open(
                  waLink(
                    settings.whatsapp,
